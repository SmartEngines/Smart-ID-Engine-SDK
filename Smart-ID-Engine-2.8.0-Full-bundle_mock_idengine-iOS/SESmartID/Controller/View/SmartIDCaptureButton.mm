...
- (void) restoreState {
  if ([self mode]  == SECameraButtonModeVideo) {
    NSLog(@"SECameraButtonModeVideo");
  } else if ([self mode]  == SECameraButtonModePhoto) {
    NSLog(@"SECameraButtonModePhoto");
  }
  if ([self mode] == SECameraButtonModeVideo) {
    [self setRecordingState:SECameraButtonStateWaiting];
    [self animateEndRecordingWithCompletion:nil duration:0.0];
...
- (void) setNewMode:(SECameraButtonMode)mode {
  [self setMode:mode];
  [self restoreState];
  
  // Update the button's appearance based on the mode
  if (mode == SECameraButtonModePhoto) {
    [self.layer setBorderColor:[UIColor grayColor].CGColor]; // Set to gray for photo mode
  } else {
    [self.layer setBorderColor:[self defaultColor].CGColor]; // Set to white for video mode
  }
}
