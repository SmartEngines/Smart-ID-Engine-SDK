...
- (instancetype) initWithString:(NSString *)string {
  ...
  NSError *error = nil;
  NSArray *parsedArray = [NSJSONSerialization JSONObjectWithData:data options:kNilOptions error:&error];
  
  if (error || ![parsedArray isKindOfClass:[NSArray class]]) {
    NSException* exc = [NSException
        exceptionWithName:@"InvalidArgumentException"
        reason:@"Invalid point array format"
        userInfo:nil];
    @throw exc;
    return nil;
  }
  ...
  for (NSArray *coords in parsedArray) {
    if (coords.count != 2) {
      NSException* exc = [NSException
          exceptionWithName:@"InvalidArgumentException"
          reason:@"Each point must have 2 coordinates"
          userInfo:nil];
      @throw exc;
      return nil;
    }
    SECommonPoint *point = [[SECommonPoint alloc] initWithX:[coords[0] doubleValue] withY:[coords[1] doubleValue]];
