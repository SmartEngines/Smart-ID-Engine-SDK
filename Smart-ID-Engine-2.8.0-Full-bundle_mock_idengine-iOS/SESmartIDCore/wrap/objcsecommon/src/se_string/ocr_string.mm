...
- (NSArray<SECommonOcrPair *> *) GetPairTable {
  try {
    se::common::OcrPair* ptr = internal->GetPairTable();
    if (!ptr)
      return nil;

    const int k = internal->GetCharsCount();
    const int variants = 8;
    const int total = k * variants;

    NSMutableArray<SECommonOcrPair *> *out =
        [NSMutableArray arrayWithCapacity:(NSUInteger)total];

    for (int idx = 0; idx < total; ++idx) {
      [out addObject:[[SECommonOcrPair alloc] initFromInternalOcrPair:ptr[idx]]];
    }
