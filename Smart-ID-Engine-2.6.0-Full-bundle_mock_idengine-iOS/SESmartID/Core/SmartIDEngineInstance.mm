...
- (void) initVideoSession {
  ...
  @synchronized (self.videoSession) {
    ...
    BOOL isActivated = [self->videoSession isActivated];
    if(isActivated) {
    ...
    [self.initializationDelegate SmartIDEngineDidActivationRequest:videoSession.getActivationRequest withCompletionHandler:^(NSString * _Nonnull actKey) {
      if (actKey != nil) {
        // Activate session
        [self->videoSession activate:actKey];
...
- (void) processSingleImageAsync:(UIImage *)image
  ...
  BOOL isActivated = [imageSession isActivated];
  if(isActivated) {
    [self processAsyncImageHelper:image withSession:imageSession withCompletionHandler:callback];
    return;
  }
  
  NSLog(@"Session activation...");
  [self.initializationDelegate SmartIDEngineDidActivationRequest:imageSession.getActivationRequest
                                           withCompletionHandler:^(NSString * _Nonnull actKey) {
    
    if (actKey == nil) { NSLog(@"Empty activation key"); return; }
    // Activate session
    [imageSession activate:actKey];
    
    if(![imageSession isActivated]){
...
- (void) processAsyncImageHelper:(UIImage *)image withSession:(SEIdSession *)imageSession withCompletionHandler:(nullable void(^)(SEIdResult *))callback {
  ...
  // Otherwise use default callbacks
  if ([NSThread isMainThread]) {
