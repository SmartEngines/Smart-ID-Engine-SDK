...
fun NfcDataScreen(
    data: PassportData,
    checkState: NfcDataCheckState
){

    Column(Modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally) {
        Header(text = "DATA CHECKS")
        when(checkState){
            NfcDataCheckState.Empty     -> {Text("no data")}// impossible case
            NfcDataCheckState.Working   -> {Text("Loading...")}
            is NfcDataCheckState.Error  -> {Text( checkState.error, color = MaterialTheme.colorScheme.onErrorContainer, modifier = Modifier.background(color = MaterialTheme.colorScheme.errorContainer, shape = RoundedCornerShape(10.dp)).padding(10.dp) )}
            is NfcDataCheckState.Ready  -> {
                if(checkState.checks.isEmpty()) Text("No data")
                else checkState.checks.forEach { (key, value) ->
                    DataRow(key, value);
                }
            }
        }
...
private fun DataRow(label:String, value:String){
    Row(
        ...
        Text(modifier = Modifier.weight(2.5f),
...
//--------------------------------------------------------------------------------------------------
// PREVIEW
private val mockPassportData : PassportData
    get() = PassportData(
        mrzInfo = MRZInfo(
            "P",//documentCode
            "RUS",//issuingState
...
@Composable
private fun NfcDataScreen_Preview1() {
    val passportData = remember { mockPassportData }
    Kotlin_sampleTheme(darkTheme = true) {
        Surface{
            NfcDataScreen(passportData, NfcDataCheckState.Working)
        }
    }
}
@Preview(showBackground = true)
@Composable
private fun NfcDataScreen_Preview2() {
    val passportData = remember { mockPassportData }
    Kotlin_sampleTheme(darkTheme = true) {
        Surface{
            NfcDataScreen(passportData, NfcDataCheckState.Error("Data checking error"))
        }
    }
}
