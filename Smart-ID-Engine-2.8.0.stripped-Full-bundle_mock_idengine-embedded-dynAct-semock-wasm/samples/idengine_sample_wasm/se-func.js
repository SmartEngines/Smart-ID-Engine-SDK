...
function prepareImageBuffer(SE, requestedSize) {
  ...
  const cap = Math.max(requestedSize * 1.5, 8 * 1024 * 1024);
  // console.log("Cap: ",cap);
...
const faceMatching = async (faceData, engine, ENGINE_CONFIG, SE) => {
  ...
  const faceResult = faceSession.GetSimilarity(faceA, faceB);

  const similarity_estimation = faceResult.GetSimilarityEstimation();
  const similarity_result = faceResult.GetSimilarity();

  let similarity =
    similarity_result === SE.seIdFaceSimilarity.Uncertain ? "uncertain" :
    similarity_result === SE.seIdFaceSimilarity.Same      ? "same" :
    similarity_result === SE.seIdFaceSimilarity.Different ? "different" :
    "";

  return{
    requestType: "faceResult",
    similarity_estimation: similarity_estimation.toFixed(4),
...
async function recognizeFile(data, ENGINE, ENGINE_CONFIG, SE) {
  ...
  const resultMessage = resultObject(result, image.GetSize(), SE);
...
function recognizeFrame(image, spawnedSessionRGBA, SE) {
...
function resultObject(result, templateSize, SE) {
  ...
  for (let i = 0; i < result.GetTemplateDetectionResultsCount(); i++) {
    ...
    tmp_dict = {
      "confidence": templateResult.GetConfidence(),
      "is_rejected": !templateResult.GetIsAccepted(),
      "template_quad": getArrFromPoints(templateResult.GetQuadrangle()),
      "template_size": templateResult.GetStandardSize(),
    }
    templateDetection_full[templateResult.GetTemplateName()] = tmp_dict
  }

  const templateSegmentation_full = [];
  for (let i = 0; i < result.GetTemplateSegmentationResultsCount(); i++) {
    ...
    for (; !qmi.Equals(sr.RawFieldQuadranglesEnd()); qmi.Advance()) {
      const quad = qmi.GetValue();
      const key = qmi.GetKey();
      // console.log(qmi)
      tmp_quad = {
        "quad": getArrFromPoints(quad),
        "key": key,
      }
      segm_quads.push(tmp_quad);
    }

    tmp_dict = {
      "confidence": sr.GetConfidence(),
      "is_rejected": !sr.GetIsAccepted(),
      "quad": segm_quads
    }

    templateSegmentation_full.push(tmp_dict)
  }

  return {
    requestType: "result",
    docType: result.GetDocumentType(),
    data: getTextFields(result, SE),
    images: getImageFields(result, templateDetection, templateSegmentation, templateSize),
    templates: templateDetection_full,
    templateSegmentation: templateSegmentation_full,
    image_size: templateSize,
    time: timeDiff(),
  };
}

// Get text fields in result
function getTextFields(result, SE) {
  const data = {};
  ...
  for (; !tf.Equals(result.TextFieldsEnd()); tf.Advance()) {
    ...
    const cf = result.ForensicCheckFieldsBegin();
    for (; !cf.Equals(result.ForensicCheckFieldsEnd()); cf.Advance()){
        const checkName = cf.GetValue().GetName();
        const checkStatus = cf.GetValue().GetValue();
        reason = ""
        checkStatusString = "";
        if (checkStatus == SE.seIdCheckStatus.IdCheckStatus_Undefined) {
            checkStatusString = "undefined"
            if (cf.GetValue().GetBaseFieldInfo().HasAttribute("reason")){
                reason = ", reason: " + cf.GetValue().GetBaseFieldInfo().GetAttribute("reason")
            }
        }
        else if (checkStatus == SE.seIdCheckStatus.IdCheckStatus_Passed){
            checkStatusString = "passed"
        }
        else if (checkStatus == SE.seIdCheckStatus.IdCheckStatus_Failed){
            checkStatusString = "failed"
        }
        totalString = checkStatusString + reason
        data[checkName] = {
            name: checkName,
            value: totalString
        }
