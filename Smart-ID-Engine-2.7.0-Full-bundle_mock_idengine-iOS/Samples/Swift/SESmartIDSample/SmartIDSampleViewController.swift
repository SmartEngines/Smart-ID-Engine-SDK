    ...
    if passport != nil {
      ...
      if self.doctyperaw != nil && passport!.passportImage != nil {
        ...
        do {
          ...
          let checks = ["check_4000000000001", "check_4000000000002"]
          ...
          if r.getRef().getForensicCheckFieldsCount() > 0 {
            ...
            while !fr_i.isEqual(toIter: r.getRef().forensicCheckFieldsEnd()) {
              ...
              if checks.contains(fr_i.getKey()) {
                if let index = resultTextFields.firstIndex(where: { $0.fieldName == fr_i.getKey() }) {
                  self.resultTextFields[index] = (fr_i.getKey(), value)
                }
              }
              fr_i.advance()
            }
      ...
      if passport!.passportImage != nil {
        ...
        self.resultImageFields.sort(by: {
            return $0.0 < $1.0
        })
      }
      self.nfcButton.isEnabled = false
  ...
  func pickSelfie(image: UIImage) {
    ...
    func selfieCallback( similarityResult: SEIdFaceSimilarityResult ) {
      ...
      for i in 0..<resultTextFields.count {
        if resultTextFields[i].fieldName == "Selfie check score" || resultTextFields[i].fieldName == "Selfie check verdict" {
      ...
      switch similarity {
      case SEDifferent:
        similarityText = "Different"
      case SEUncertain:
        similarityText = "Uncertain"
      case SESame:
        similarityText = "Same"
      default:
        similarityText = "Unknown status"
      }
      self.resultTextFields.append(("Selfie check score", "\(similarityEstimation)"))
      self.resultTextFields.append(("Selfie check verdict", "\(similarityText)"))
      self.resultTextFields.sort(by: {
          return $0.0 < $1.0
      })
      self.resultTableView.reloadData()
