    ...
    public void initSession(Context context, StateCallback callback) {
        ...
        SessionExecutor.execute(() -> {
            try {
                ...
                // 2.4 Define if ROI required
                SettingsStore.isRoi = true;
                StringsSetIterator iterator = sessionSettings.EnabledDocumentTypesBegin();
                StringsSetIterator end      = sessionSettings.EnabledDocumentTypesEnd();
                while (!iterator.Equals(end)){
                    String docType = iterator.GetValue();
                    if(docType.contains("phone")||docType.contains("card_number")) {
                        iterator.Advance();
                    }else{
                        SettingsStore.isRoi = false;
                        break;
                    }
                }

                // 3. Spawn recognition session
                session = engine.SpawnSession(sessionSettings, SIGNATURE, idCallback);
