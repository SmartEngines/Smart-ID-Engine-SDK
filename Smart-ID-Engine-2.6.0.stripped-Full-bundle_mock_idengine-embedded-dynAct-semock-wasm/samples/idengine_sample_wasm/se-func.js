...
const faceMatching = async (faceData, engine, ENGINE_CONFIG, SE) => {
  ...
  const face_score = faceSession.GetSimilarity(faceA, faceB);

  const face_string = face_score.toFixed(4);

  return { requestType: "faceResult", score: face_string, time: timeDiff() };
};
