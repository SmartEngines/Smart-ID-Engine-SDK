  ...
  public static void OutputRecognitionResult(IdResult result, 
    ...
    for (int i = 0; i < result.GetTemplateDetectionResultsCount(); i++) {
      ...
      if (tpl_res.GetAttributesCount() > 0) {
        ...
        for (StringsMapIterator it = tpl_res.AttributesBegin();
             !it.Equals(tpl_res.AttributesEnd());
             it.Advance()) {
    ...
    for (IdCheckFieldsMapIterator it = result.ForensicCheckFieldsBegin(); 
        ...
        if (checkStatus.equals("undefined") && field.GetBaseFieldInfo().HasAttribute("reason")) {
            System.out.printf("%-36sreason: %s\n", 
                              "", 
