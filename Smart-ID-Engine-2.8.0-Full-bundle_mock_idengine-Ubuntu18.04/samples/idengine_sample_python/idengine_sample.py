...
def OutputRecognitionResult(recog_result,session_settings):
    ...
    for i in range(recog_result.GetTemplateDetectionResultsCount()):
      ...
      if tpl_res.GetAttributesCount() > 0:
        ...
        while(f_it != tpl_res.AttributesEnd()):
          print('        {0}: {1}'.format(f_it.GetKey(), f_it.GetValue()))
          f_it.Advance()
    ...
    while(f_it != recog_result.TextFieldsEnd()):
      field = f_it.GetValue()
      is_accepted = "[+]" if field.GetBaseFieldInfo().GetIsAccepted() else "[-]" 
      print(f"    {field.GetName():<25} {is_accepted}  ({field.GetBaseFieldInfo().GetConfidence():.3f}) {field.GetValue().GetFirstString().GetCStr()}")
      f_it.Advance()

    print('Image fields:')
    f_it = recog_result.ImageFieldsBegin()
    while(f_it != recog_result.ImageFieldsEnd()):
      field = f_it.GetValue()
      is_accepted = "[+]" if field.GetBaseFieldInfo().GetIsAccepted() else "[-]" 
      print(f"    {field.GetName():<25} {is_accepted}  ({field.GetBaseFieldInfo().GetConfidence():.3f}) W: {field.GetValue().GetWidth()} H: {field.GetValue().GetWidth()}")
      f_it.Advance()

    print('Forensic check fields:')
    f_it = recog_result.ForensicCheckFieldsBegin()
    while(f_it != recog_result.ForensicCheckFieldsEnd()):
      field = f_it.GetValue()
      check_status = "passed" if field.GetValue() == pyidengine.IdCheckStatus_Passed else \
                  ("failed" if field.GetValue() == pyidengine.IdCheckStatus_Failed else "undefined")
      print(f"    {field.GetName()}        {check_status}")
      if check_status == "undefined" and field.GetBaseFieldInfo().HasAttribute("reason"):
        reason = field.GetBaseFieldInfo().GetAttribute("reason")
        print(f"{'':<36}reason: {reason}")
      f_it.Advance()
    
    print(f"Result terminal:              {'[+]' if recog_result.GetIsTerminal() else '[-]'}")
